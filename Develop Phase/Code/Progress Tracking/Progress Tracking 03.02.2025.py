...
def main():
    ...
    search_url_google = general_check(create_user_search_parameters(user_keywords,
                                                                    user_location, base_url_google, query),
    ...
    general_check(find_job_data(soup, jobs_list, 'wHYlTd FqK3wc MKCbgd', 2), "Unable toinitialize jobsearch...")
    general_check(find_job_data(soup, jobs_list, 'Yf9oye', 3), "Unable to initialize jobsearch...")
    general_check(find_job_data(soup, jobs_list, 'nNzjpf-cS4Vcb-PvZLI-Ueh9jd-LgbsSeJyewjb - tlSJBe', 4), "Unable to initialize job search...")
    print(jobs_list)
